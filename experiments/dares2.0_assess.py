...
def get_data_frames(file_path='data/readability/DARES2.0/fine-grained', mode='default', append_column=None):
    """
    Load data from DARES2.0 folder structure (same as DARES1.0)
    Columns: Word, Grade, Filename, Text, num_sent, Arabic_Filename
    """
    sampling_random_state = 777

    train_df = pd.read_csv(f'{file_path}/train.csv', encoding='utf-8-sig')
    test_df = pd.read_csv(f'{file_path}/test.csv', encoding='utf-8-sig')
    validation_df = pd.read_csv(f'{file_path}/validation.csv', encoding='utf-8-sig')
    ...
    if append_column == 'word_file_cat' or append_column == 'word_file':
        ...
        validation_df['text'] = validation_df['Word'] + " : " + validation_df['Arabic_Filename'] + " : " + validation_df['text']
    elif append_column:
...
def run(args):
    if args.run_mode == "test":
        ...
        train_df, test_df, validation_df = get_data_frames(file_path='data/readability/DARES2.0/coarse-grained', mode='test')
    elif args.run_mode == "balanced":
        train_df, test_df, validation_df = get_balanced_data_frames()
    # Fine-grained experiments (G1-G12)
    elif args.run_mode == "raw":
        train_df, test_df, validation_df = get_data_frames(file_path='data/readability/DARES2.0/fine-grained')
        ...
        train_df, test_df, validation_df = get_data_frames(file_path='data/readability/DARES2.0/fine-grained',
                                                           append_column=args.append_column)
    # Coarse-grained experiments (EE/UE/INT/HS)
    elif args.run_mode == "raw_cat":
        train_df, test_df, validation_df = get_data_frames(file_path='data/readability/DARES2.0/coarse-grained')
    ...
    for i in range(args.n_fold):
        ...
        if args.save_predictions:
            test_df.to_csv(f'run_statistics/run_mode_{args.run_mode}_predictions_model_{m_name}_run_no_{i}.csv',
                           sep='\t',
                           index=False)
    with open(f'final_stats.tsv', 'a') as f:
        ...
        f.write(
            f'{args.model_name}\t{args.run_mode}\t{np.mean(w_f1_lst)}\t{np.std(w_f1_lst)}\t{np.mean(m_f1_lst)}\t{np.std(m_f1_lst)}\n')
