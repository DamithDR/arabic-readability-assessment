...
def get_data_frames(file_path='data/readability/data_V2.0', mode='default', exp_type='C', append_column=None):
    ...
    if exp_type == 'C':
        ...
        train_df.rename(columns={'Text': 'text', 'Fine-grained': 'labels'}, inplace=True)
        test_df.rename(columns={'Text': 'text', 'Fine-grained': 'labels'}, inplace=True)
        validation_df.rename(columns={'Text': 'text', 'Fine-grained': 'labels'}, inplace=True)
    ...
    if append_column == 'word_file_cat' or 'word_file':
        ...
        validation_df['text'] = validation_df['Word'] + " : " + validation_df['Arabic_Filename'] + " : " + \
                                validation_df[
                                    'text']
    elif append_column:
